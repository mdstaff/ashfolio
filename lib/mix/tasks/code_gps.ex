defmodule Mix.Tasks.CodeGps do
  ...
  defp check_expense_dashboard_integration(live_views) do
    dashboard = Enum.find(live_views, fn lv -> 
  ...
  defp extract_assigns(content) do
    ...
    |> Enum.map(&List.last/1)
    |> Enum.uniq()
    |> Enum.take(10)  # Limit to avoid noise
  end
  ...
  defp extract_component_attrs(content, component_name) do
    ...
    start_line = Enum.find_index(lines, fn line ->
  ...
  defp encode_components_with_attrs(components) do
    ...
    |> Enum.take(8)  # Limit to most important
    |> Enum.map(fn comp ->
      attrs_str = if length(comp.attrs) > 0 do
  ...
  defp _encode_live_views_concise(live_views) do
    ...
    |> Enum.map(fn lv ->
      name = lv.name |> String.replace("AshfolioWeb.", "") |> String.replace("Live", "")
      events_str = if length(lv.events) > 0, do: " events: #{inspect(lv.events)}", else: ""
      missing_str = if length(lv.missing_subscriptions) > 0, do: " MISSING: #{inspect(lv.missing_subscriptions)}", else: ""
      
      "#{name}: #{lv.file}:#{lv.mount_line}#{events_str}#{missing_str}"
    end)
    |> Enum.join("\n")
  end
  ...
  defp _encode_key_components(components) do
    ...
    components
    |> Enum.filter(fn comp -> 
      comp.usage_count > 2 or 
      String.contains?(comp.name, "card") or
      String.contains?(comp.name, "button") or
      String.contains?(comp.name, "form")
    end)
    |> Enum.map(fn comp ->
      "#{comp.name}: #{comp.file}:#{comp.line} (used #{comp.usage_count}x)"
    end)
    |> Enum.join("\n")
  end
  ...
  defp _encode_suggestions_concise(suggestions) do
    ...
    |> Enum.map(fn sugg ->
      ...
      |> Enum.map(fn step -> "#{step.action} at #{step.file}:#{step[:line] || "?"}" end)
      |> Enum.join("; ")
      
      "#{sugg.name}: #{sugg.description}\n  TODO: #{steps_summary}"
    end)
    |> Enum.join("\n\n")
  end
    ...
    |> Enum.map(fn lv ->
      """
        #{slugify(lv.name)}:
          name: "#{lv.name}"
          file: "#{lv.file}"
          mount_line: #{lv.mount_line || "null"}
          render_line: #{lv.render_line || "null"}
          events: #{inspect(lv.events)}
          assigns: #{inspect(lv.assigns)}
          subscriptions: #{inspect(lv.subscriptions)}
          missing_subscriptions: #{inspect(lv.missing_subscriptions)}
      """
    end)
    |> Enum.join("\n")
  end
    ...
    |> Enum.map(fn test ->
      """
        #{slugify(test.name)}:
          ...
          tested_module: "#{test.tested_module}"
          describes: #{inspect(test.describes)}
      """
    end)
    |> Enum.join("\n")
  end
  ...
  defp _encode_suggestions(suggestions) when is_list(suggestions) do
    if length(suggestions) == 0 do
      ...
      suggestions
      |> Enum.map(fn sugg ->
        steps_yaml = sugg.steps
        |> Enum.with_index()
        |> Enum.map(fn {step, idx} ->
          """
              - step: #{idx + 1}
                action: "#{step.action}"
                file: "#{step.file}"#{if step[:line], do: "\n        line: #{step.line}", else: ""}#{if step[:after_function], do: "\n        after_function: \"#{step.after_function}\"", else: ""}#{if step[:code], do: "\n        code: |\n          #{step.code}", else: ""}
          """
        end)
        |> Enum.join("")
