defmodule Ashfolio.Portfolio.Transaction do
  ...
  actions do
    ...
    create :create do
      ...
      accept [:type, :quantity, :price, :total_amount, :fee, :date, :notes, :account_id, :symbol_id]
      primary? true
  ...
  defp validate_quantity_for_type(changeset) do
    ...
    cond do
      type == :buy and (is_nil(quantity) or Decimal.compare(quantity, 0) != :gt) ->
        Ash.Changeset.add_error(changeset, field: :quantity, message: "Quantity must be positive for buy transactions")

      type == :sell and (is_nil(quantity) or Decimal.compare(quantity, 0) != :lt) ->
        Ash.Changeset.add_error(changeset, field: :quantity, message: "Quantity must be negative for sell transactions")
