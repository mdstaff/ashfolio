defmodule Ashfolio.Portfolio.Symbol do
  ...
  validations do
    ...
    validate match(:symbol, ~r/^[A-Z0-9\-\.]+$/), message: "Symbol must contain only uppercase letters, numbers, dashes, and dots"
  ...
  actions do
    ...
    create :create do
      ...
      accept [:symbol, :name, :asset_class, :currency, :isin, :sectors, :countries, :data_source, :current_price, :price_updated_at]
      primary? true
    end
    ...
    read :stale_prices do
      ...
      prepare fn query, _context ->
        ...
        threshold = case Ash.Query.get_argument(query, :stale_threshold) do
          nil -> DateTime.utc_now() |> DateTime.add(-3600, :second)  # 1 hour ago
          datetime -> datetime
        end
