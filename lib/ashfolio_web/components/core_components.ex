defmodule AshfolioWeb.CoreComponents do
  @moduledoc """
  ...
  def table(assigns) do
    ...
    ~H"""
    <div class={["overflow-y-auto px-4 sm:overflow-visible sm:px-0", Map.get(assigns, :class)]}>
      <table class="w-[40rem] mt-11 sm:w-full">
        <thead class="text-sm text-left leading-6 text-zinc-500">
          <tr>
            <th :for={col <- @col} class={["p-0 pb-4 pr-6 font-normal", Map.get(col, :class)]}>
              {col.label}
            </th>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(AshfolioWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(AshfolioWeb.Gettext, "errors", msg, opts)
    end
  end
