defmodule AshfolioWeb.CodeGpsIntegrationTest do
  ...
  describe "Code GPS generation" do
    ...
    test "output contains expected structure and content" do
      ...
      assert content =~ "# === INTEGRATION OPPORTUNITIES ===", "Should contain integration section"
  ...
  describe "Claude agent integration" do
    ...
    test "Code GPS identifies specific next actions for agents" do
      ...
      assert content =~ "add_expense_to_dashboard", "Should suggest expense integration"
      assert content =~ "Dashboard missing expense data integration", "Should explain what's missing"
    ...
    test "validates workflow: agent should run Code GPS before any development" do
      ...
      assert Enum.any?(lines, &String.contains?(&1, "=== LIVE VIEWS ===")), "Should have LiveViews section"
      assert Enum.any?(lines, &String.contains?(&1, "=== KEY COMPONENTS ===")), "Should have components section"
      assert Enum.any?(lines, &String.contains?(&1, "=== INTEGRATION OPPORTUNITIES ===")), "Should have integration section"
