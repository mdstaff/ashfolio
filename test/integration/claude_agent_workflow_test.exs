defmodule AshfolioWeb.ClaudeAgentWorkflowTest do
  ...
  test "complete Claude agent workflow simulation" do
    ...
    dashboard_section = lines
      |> Enum.drop_while(fn line -> not String.starts_with?(line, "Dashboard:") end)
      |> Enum.take_while(fn line -> not String.match?(line, ~r/^[A-Z]/) or String.starts_with?(line, "Dashboard:") end)
    ...
    integration_section = lines
      |> Enum.drop_while(fn line -> not String.contains?(line, "INTEGRATION OPPORTUNITIES") end)
      |> Enum.take(20)  # Take reasonable section
    
    expense_integration = Enum.find(integration_section, &String.contains?(&1, "add_expense_to_dashboard"))
  ...
  test "workflow performance meets agent requirements" do
    ...
    File.read!(@code_gps_path)  # Read step
  ...
  test "manifest provides enough context to avoid reading multiple files" do
    ...
    essential_sections = [
      "LIVE VIEWS",  # Architecture overview
      "KEY COMPONENTS", # UI building blocks
      "INTEGRATION OPPORTUNITIES" # Next actions
    ]
    ...
    essential_data = [
      "events:",     # Available interactions
      "subscriptions:", # Data dependencies
      "missing:",    # Integration gaps
      "file:",       # Navigation paths
      "mount:",      # Code locations
    ...
    assert line_count < 100, "Should be concise enough for AI consumption, was #{line_count} lines"
  end
end