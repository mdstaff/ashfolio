defmodule Ashfolio.Integration.CategoryWorkflowIntegrationTest do
  ...
  describe "investment category assignment workflows" do
    setup do
      ...
      {:ok, user: user, account: account, symbol: symbol}
    end
    
    test "assign category during transaction creation", %{user: user, account: account, symbol: symbol} do
    ...
    test "bulk category assignment to existing transactions", %{user: user, account: account, symbol: symbol} do
      ...
      updated_count = Enum.reduce(transaction_ids, 0, fn transaction_id, acc ->
    ...
    test "filter transactions by category", %{user: user, account: account, symbol: symbol} do
      # Get categories
      {:ok, categories} = TransactionCategory.categories_for_user(user.id)
      growth_category = Enum.find(categories, &(&1.name == "Growth"))
      income_category = Enum.find(categories, &(&1.name == "Income"))
      ...
      {:ok, growth_tx1} = Transaction.create(%{
        account_id: account.id,
        symbol_id: symbol.id,
        type: :buy,
      ...
      {:ok, growth_tx2} = Transaction.create(%{
        account_id: account.id,
        symbol_id: symbol.id,
        type: :sell,
      ...
      {:ok, income_tx} = Transaction.create(%{
        account_id: account.id,
        symbol_id: symbol.id,
        type: :dividend,
      ...
      {:ok, uncategorized_tx} = Transaction.create(%{
        account_id: account.id,
        symbol_id: symbol.id,
        type: :buy,
    ...
    test "category performance reporting", %{user: user, account: account, symbol: symbol} do
      # Get categories
      {:ok, categories} = TransactionCategory.categories_for_user(user.id)
      growth_category = Enum.find(categories, &(&1.name == "Growth"))
      income_category = Enum.find(categories, &(&1.name == "Income"))
      ...
      {:ok, _dividend} = Transaction.create(%{
        account_id: account.id,
        symbol_id: symbol.id,
        type: :dividend,
      ...
      assert Decimal.equal?(growth_stats.total_invested, Decimal.new("30500.00")) # 20000 + 10500
      assert Decimal.equal?(growth_stats.total_proceeds, Decimal.new("5500.00"))  # 25 * 220
  ...
  describe "system category protection" do
    ...
    test "system categories cannot be deleted", %{user: user} do
      {:ok, categories} = TransactionCategory.categories_for_user(user.id)
      ...
      {:ok, updated_categories} = TransactionCategory.categories_for_user(user.id)
      still_exists = Enum.find(updated_categories, &(&1.id == system_category.id))
      assert still_exists != nil
    end
    
    test "system categories cannot be edited", %{user: user} do
      {:ok, categories} = TransactionCategory.categories_for_user(user.id)
      system_category = Enum.find(categories, &(&1.is_system == true))
      
      # Attempt to modify system category should fail or be ignored
      result = TransactionCategory.update(system_category, %{
    ...
    test "custom categories can be modified and deleted", %{user: user} do
      # Create custom category
      {:ok, custom_category} = TransactionCategory.create(%{
        name: "Custom Growth",
        color: "#FF5733",
        is_system: false,
        user_id: user.id
      ...
      {:ok, updated} = TransactionCategory.update(custom_category, %{
  ...
  describe "category workflow performance" do
    ...
    test "category filtering performance with many transactions", %{user: user, account: account, symbol: symbol} do
      # Get category for assignment
      {:ok, categories} = TransactionCategory.categories_for_user(user.id)
      category = hd(categories)
      
      # Create many transactions in the same category
      _transactions = for i <- 1..100 do
        transaction_type = if(rem(i, 2) == 0, do: :buy, else: :sell)
        quantity = if(transaction_type == :sell, do: Decimal.new("-#{i}"), else: Decimal.new("#{i}"))
    ...
    test "category statistics calculation performance", %{user: user, account: account, symbol: symbol} do
      ...
      category_stats = for category <- categories do
