defmodule AshfolioWeb.Integration.CriticalIntegrationPointsTest do
  ...
  describe "Transaction Impact on Portfolio" do
    test "buy transaction increases portfolio value correctly", %{user: user, account: account, symbol: symbol} do
    ...
    test "sell transaction decreases holdings correctly", %{user: user, account: account, symbol: symbol} do
      # First create a buy transaction
      {:ok, _buy_tx} = Transaction.create(%{
        type: :buy,
        account_id: account.id,
        symbol_id: symbol.id,
      ...
      {:ok, _sell_tx} = Transaction.create(%{
        type: :sell,
        account_id: account.id,
        symbol_id: symbol.id,
        quantity: Decimal.new("-10"), # Negative for sell
    ...
    test "dividend transaction adds to portfolio returns", %{account: account, symbol: symbol} do
      ...
      {:ok, dividend_tx} = Transaction.create(%{
        type: :dividend,
        account_id: account.id,
        symbol_id: symbol.id,
        quantity: Decimal.new("1"), # Positive for dividends
        price: Decimal.new("2.50"),
        total_amount: Decimal.new("25.00"), # $2.50 per share for 10 shares
        date: ~D[2024-08-07]
      })
  ...
  describe "Error Handling Scenarios" do
    test "handles invalid transaction data gracefully", %{account: account, symbol: symbol} do
      ...
      result = Transaction.create(%{
        ...
        account_id: account.id,
        symbol_id: symbol.id,
        quantity: Decimal.new("-10"), # Invalid: negative quantity for buy
        price: Decimal.new("100.00"),
        total_amount: Decimal.new("1000.00"),
        date: ~D[2024-08-07]
      })
