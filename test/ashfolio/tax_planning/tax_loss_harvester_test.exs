defmodule Ashfolio.TaxPlanning.TaxLossHarvesterTest do
  ...
  describe "identify_opportunities/3" do
    ...
    test "filters out positions with insufficient losses" do
      account_id = "test-account"
      # High threshold
      loss_threshold = Decimal.new("2000.00")
  ...
  describe "recommend_replacements/3" do
    ...
    test "filters replacements by suitability score" do
      # Would have low suitability replacements
      symbol = "OBSCURE"
    ...
    test "limits replacement suggestions to reasonable number" do
      # Popular ETF with many similar options
      symbol = "VTI"
    ...
    test "considers allocation targets in recommendations" do
      symbol = "AAPL"
      # 10% allocation
      allocation_target = Decimal.new("0.10")

      expect(Ashfolio.ContextMock, :read, fn Symbol, :find_by_symbol, _args ->
        {:ok, [%{id: "uuid-1", symbol: symbol, name: "Apple Inc."}]}
      end)
  ...
  describe "check_wash_sale_compliance/4" do
    test "validates compliant replacement transaction" do
      sell_symbol = "AAPL"
      # Different company, should be compliant
      buy_symbol = "MSFT"
    ...
    test "detects wash sale violation with substantially identical securities" do
      sell_symbol = "VTI"
      # Both total market ETFs - substantially identical
      buy_symbol = "ITOT"
      transaction_date = Date.utc_today()
    ...
    test "provides safe transaction date for wash sale avoidance" do
      sell_symbol = "AAPL"
      # Same symbol - definitely wash sale
      buy_symbol = "AAPL"
      transaction_date = Date.utc_today()
    ...
    test "assesses symbol similarity correctly" do
      ...
      test_cases = [
        # Both S&P 500 ETFs
        {"SPY", "IVV", :high_similarity},
        # Both total market ETFs  
        {"VTI", "ITOT", :high_similarity},
        # Different companies
        {"AAPL", "MSFT", :low_similarity},
        # Same symbol
        {"AAPL", "AAPL", :identical}
      ]
  ...
  describe "optimize_harvest_strategy/3" do
    test "creates optimal harvesting sequence" do
      ...
      assert {:ok, strategy} =
               TaxLossHarvester.optimize_harvest_strategy(
                 portfolio_targets,
                 tax_rate
               )
  ...
  describe "error handling and edge cases" do
    ...
    test "validates input parameters" do
      ...
      assert {:ok, opportunities} =
               TaxLossHarvester.identify_opportunities(
                 "test-account",
                 # Negative threshold
                 Decimal.new("-100")
               )
