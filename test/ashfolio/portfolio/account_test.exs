defmodule Ashfolio.Portfolio.AccountTest do
  ...
  describe "Account resource" do
    test "can create account with required attributes", %{user: user} do
      ...
      assert account.is_excluded == false  # Default value
      assert Decimal.equal?(account.balance, Decimal.new(0))  # Default value
    ...
    test "can update account attributes", %{user: user} do
      ...
      {:ok, updated_account} = Ash.update(account, %{
    ...
    test "validates required name field", %{user: user} do
      {:error, changeset} = Ash.create(Account, %{
        platform: "Test",
        user_id: user.id
      })
  ...
  describe "Account actions" do
    test "active action returns only non-excluded accounts", %{user: user} do
      ...
      {:ok, active_account} = Ash.create(Account, %{
        name: "Active Account",
        is_excluded: false,
        user_id: user.id
      })
      ...
      {:ok, _excluded_account} = Ash.create(Account, %{
    ...
    test "by_user action returns accounts for specific user", %{user: user} do
      ...
      {:ok, user_account} = Ash.create(Account, %{
        name: "User Account",
        user_id: user.id
      })

      # Create account for other user
      {:ok, _other_account} = Ash.create(Account, %{
        name: "Other Account",
    ...
    test "toggle_exclusion action works correctly", %{user: user} do
      ...
      {:ok, updated_account} = Ash.update(account, %{is_excluded: true}, action: :toggle_exclusion)
    ...
    test "update_balance action works correctly", %{user: user} do
      ...
      {:ok, updated_account} = Ash.update(account, %{balance: Decimal.new("2000.00")}, action: :update_balance)
  ...
  describe "Account code interface" do
    ...
    test "list function works", %{user: user} do
      {:ok, _account} = Ash.create(Account, %{
        name: "Test Account",
        user_id: user.id
      })
