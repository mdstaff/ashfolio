defmodule Ashfolio.Portfolio.Services.CorporateActionApplierTest do
  ...
  alias Ashfolio.Portfolio.{CorporateAction, TransactionAdjustment}
  ...
  describe "apply_corporate_action/1" do
    setup do
      ...
      tx1 = Ashfolio.SQLiteHelpers.create_test_transaction(account, symbol, %{
        type: :buy,
      ...
      tx2 = Ashfolio.SQLiteHelpers.create_test_transaction(account, symbol, %{
        type: :buy,
    ...
    test "applies stock split to all transactions", %{symbol: symbol, transactions: _transactions} do
      ...
      {:ok, corporate_action} = CorporateAction.create(%{
    ...
    test "handles unsupported action type", %{symbol: symbol} do
      {:ok, corporate_action} = CorporateAction.create(%{
        action_type: :spinoff, # Not yet implemented
        symbol_id: symbol.id,
        ex_date: ~D[2024-06-01],
  ...
  describe "batch_apply_pending/1" do
    setup do
      ...
      _tx = Ashfolio.SQLiteHelpers.create_test_transaction(account, symbol, %{
        type: :buy,
        quantity: Decimal.new("100"),
    ...
    test "applies all pending actions for a symbol", %{symbol: symbol} do
      ...
      {:ok, split_action} = CorporateAction.create(%{
        action_type: :stock_split,
        symbol_id: symbol.id,
        ex_date: ~D[2024-05-01], # Earlier date
      ...
      {:ok, dividend_action} = CorporateAction.create(%{
        action_type: :cash_dividend,
        symbol_id: symbol.id,
        ex_date: ~D[2024-06-01], # Later date
      ...
      assert batch_results.total_adjustments == 2 # Split + dividend
    ...
    test "applies actions in chronological order by ex_date", %{symbol: symbol} do
      ...
      {:ok, later_action} = CorporateAction.create(%{
        action_type: :stock_split,
        symbol_id: symbol.id,
        ex_date: ~D[2024-06-01],
      ...
      {:ok, earlier_action} = CorporateAction.create(%{
        action_type: :cash_dividend,
        symbol_id: symbol.id,
  ...
  describe "reverse_application/1" do
    setup do
      ...
      tx = Ashfolio.SQLiteHelpers.create_test_transaction(account, symbol, %{
        type: :buy,
