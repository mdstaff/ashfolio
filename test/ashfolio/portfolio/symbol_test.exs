defmodule Ashfolio.Portfolio.SymbolTest do
  ...
  describe "Symbol resource" do
    test "can create symbol with required attributes" do
      ...
      assert symbol.sectors == []  # Default value
      assert symbol.countries == []  # Default value
    ...
    test "requires symbol attribute" do
      {:error, changeset} = Ash.create(Symbol, %{
        asset_class: :stock,
        data_source: :yahoo_finance
      })
  ...
  describe "Symbol actions" do
    setup do
      ...
      {:ok, btc} = Ash.create(Symbol, %{
        symbol: "BTC-USD",
        name: "Bitcoin",
        asset_class: :crypto,
        data_source: :coingecko,
        current_price: Decimal.new("45000.00"),
        price_updated_at: DateTime.utc_now() |> DateTime.add(-7200, :second)  # 2 hours ago
      })

      {:ok, spy} = Ash.create(Symbol, %{
        symbol: "SPY",
    ...
    test "can update symbol price", %{aapl: aapl} do
      ...
      {:ok, updated_symbol} = Ash.update(aapl, %{
      ...
      # Allow for small time differences due to processing
      assert DateTime.diff(updated_symbol.price_updated_at, new_time, :millisecond) |> abs() < 1000
    end

    test "can update symbol attributes", %{spy: spy} do
      {:ok, updated_symbol} = Ash.update(spy, %{
  ...
  describe "Symbol code interface" do
    ...
    test "find_by_symbol function works" do
      {:ok, created_symbol} = Symbol.create(%{
        symbol: "AMZN",
        asset_class: :stock,
        data_source: :yahoo_finance
      })
