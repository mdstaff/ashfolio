defmodule AshfolioWeb.ExpenseLive.ImportTest do
  ...
  describe "expense import wizard" do
    ...
    test "category mapping shows existing categories", %{conn: conn, categories: categories} do
      {:ok, view, html} = live(conn, ~p"/expenses/import")
      ...
      assert html =~ "Food" # From CSV, needs mapping
    end

    test "successful import creates expenses in database", %{conn: conn, checking_account: account, categories: %{groceries: groceries}} do
      {:ok, view, html} = live(conn, ~p"/expenses/import")

      csv_content = """
      Date,Description,Amount,Category
      2024-08-15,Weekly Groceries,125.50,Food
      2024-08-10,Gas Fill Up,45.20,Transportation
      """
      ...
      |> form("#import-form", %{
        ...
        category_mapping: %{
          "Food" => groceries.id,
          "Transportation" => groceries.id  # Map to existing category for test
        }
      })
      |> render_change()
      ...
      # Submit import
      view
      |> element("button[phx-click='import']")
      |> render_click()

      # Should redirect to expenses index
      assert_redirect(view, ~p"/expenses")

      # Should create expenses in database
      ...
      assert expense1.amount == Decimal.new("125.50")
    ...
    test "import handles duplicate detection", %{conn: conn, checking_account: account, categories: %{groceries: groceries}} do
